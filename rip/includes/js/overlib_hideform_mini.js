...
if (
  typeof olInfo == "undefined" ||
  typeof olInfo.meets == "undefined" ||
  !olInfo.meets(4.1)
)
  alert("overLIB 4.10 ou depois é requerido para o HideForm Plugin.");
else {
  function generatePopUp(content) {
    if (
      ...
      olOp ||
      !olIe55 ||
      (typeof o3_shadow != "undefined" && o3_shadow) ||
      (typeof o3_bubble != "undefined" && o3_bubble)
    )
      return;
    var wd,
      ht,
  ...
  function backDropSource(width, height, Z) {
    return (
      '<iframe frameborder="0" scrolling="no" src="javascript:false;" width="' +
      width +
      '" height="' +
      height +
      '" style="z-index: ' +
      Z +
      ';filter: Beta(Style=0,Opacity=0);"></iframe>'
    );
  }
  ...
  if (
    !(olNs4 || olOp || olIe55 || navigator.userAgent.indexOf("Netscape6") != -1)
  ) {
    ...
    if (f) {
      ...
      MMStr =
        MMStr.substring(0, ls) +
        ' {runHook("placeLayer",FREPLACE);if(olHideForm)hideSelectBox();' +
        MMStr.substring(ls + (le != -1 ? le + 3 : 0));
      document.writeln(
